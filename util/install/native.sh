...
##
cd /var/tmp
git clone https://github.com/juniorojha/configuration --depth 1 --branch $CONFIGURATION_VERSION
cd configuration
...
##
cd /var/tmp/configuration/playbooks && sudo -E ansible-playbook -v -c local ./openedx_native.yml -i "localhost," $EXTRA_VARS "$@"
ansible_status=$?
